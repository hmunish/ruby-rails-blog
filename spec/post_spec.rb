...
RSpec.describe Post, type: :Model do
  let(:user) { User.new(name: 'Munish', photo: 'www.newpics.com/jpg', bio: 'First son', posts_counter: 4) }
  before { user.save }
  ...
  describe '#recent_comments' do
    it 'it should return 5 recent comments' do
      ...
      Comment.create(text: 'hey budyTwo', user_id: user.id, post_id: post.id)
      Comment.create(text: 'hey budyThree', user_id: user.id, post_id: post.id)
      Comment.create(text: 'hey budyFour', user_id: user.id, post_id: post.id)
      Comment.create(text: 'hey budyFive', user_id: user.id, post_id: post.id)
      Comment.create(text: 'hey budySix', user_id: user.id, post_id: post.id)
      Comment.create(text: 'hey budySeven', user_id: user.id, post_id: post.id)
    ...
    it 'it should return 5 recent comments' do
      ...
      Comment.create(text: 'hey budySeven', user_id: user.id, post_id: post.id)
      my_comments = post.recent_comments
      expect(my_comments.first.text).to eq('hey budySeven')
    end
  end
end